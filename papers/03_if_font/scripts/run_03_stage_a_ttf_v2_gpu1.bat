...
chcp 65001 >/dev/null
setlocal
REM 03_if_font Stage A v2 — 200k step warm-start from v1 ckpt (cuda:1).
...
set TRAIN=%PAPER_DIR%\src\if_font\configs\train_stage_a_ttf_v2.yaml
set INIT=%PAPER_DIR%\outputs\stage_a_ttf_v1\if_font_last.pt

for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddHHmmss"') do set DT=%%I
set LOG=D:\Char\ayueh\paper_reimpl\logs\03_stage_a_ttf_v2_%DT%.log
