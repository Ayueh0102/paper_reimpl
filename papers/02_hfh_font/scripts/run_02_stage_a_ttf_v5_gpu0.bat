@echo off
chcp 65001 >nul
setlocal
REM 02 Stage A v5 — warm-start from v3 ckpt at lr=1e-5 (cuda:0).
REM v4 used lr=1e-4 + OneCycleLR ramp which destroyed v3's emerging glyph
REM structure; v5 retries with 10x lower LR to preserve features while
REM continuing to refine.

set REPO=D:\Char\ayueh\paper_reimpl\repo
set PAPER_DIR=%REPO%\papers\02_hfh_font
set DATA=%PAPER_DIR%\src\hfh_font\configs\data_stage_a_ttf.yaml
set MODEL=%PAPER_DIR%\src\hfh_font\configs\model.yaml
set TRAIN=%PAPER_DIR%\src\hfh_font\configs\train_stage_a_ttf_v5.yaml
set INIT=%PAPER_DIR%\outputs\stage_a_ttf_v3\hfh_font_last.pt

for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddHHmmss"') do set DT=%%I
set LOG=D:\Char\ayueh\paper_reimpl\logs\02_stage_a_ttf_v5_gpu0_%DT%.log

set PYTHONIOENCODING=utf-8
set PYTHONUNBUFFERED=1

cd /d %PAPER_DIR%
uv run python -u -m paper_reimpl_shared.runner.entrypoint ^
    --paper hfh_font ^
    --train "%TRAIN%" --model "%MODEL%" --data "%DATA%" ^
    --data-backend lab_server --device cuda:0 ^
    --init-ckpt "%INIT%" ^
    > "%LOG%" 2>&1

endlocal
