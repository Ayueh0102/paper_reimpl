...
chcp 65001 >nul
setlocal
REM 02_hfh_font Stage A v2 — paper-faithful: frozen VAE + train U-Net.
REM Prereq: VAE pretrain ckpt at outputs/stage_vae/vae_last.pt (produced
REM by scripts/pretrain_vae.py via paper_02_vae schtask).
...
set TRAIN=%PAPER_DIR%\src\hfh_font\configs\train_stage_a_ttf_v2.yaml
set INIT=%PAPER_DIR%\outputs\stage_vae\vae_last.pt

for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddHHmmss"') do set DT=%%I
set LOG=D:\Char\ayueh\paper_reimpl\logs\02_stage_a_ttf_v2_%DT%.log
