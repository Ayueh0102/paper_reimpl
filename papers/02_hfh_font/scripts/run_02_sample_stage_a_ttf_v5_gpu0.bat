...
chcp 65001 >nul
setlocal
REM 02 Stage A v5 sample on cuda:0.

set REPO=D:\Char\ayueh\paper_reimpl\repo
set PAPER_DIR=%REPO%\papers\02_hfh_font
set CKPT=%PAPER_DIR%\outputs\stage_a_ttf_v5\hfh_font_last.pt
set FONTS=D:\Char\ayueh\paper_reimpl\data_snapshot\fonts_free
set OUT=%PAPER_DIR%\outputs\stage_a_ttf_v5\sample_grid.png

for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddHHmmss"') do set DT=%%I
set LOG=D:\Char\ayueh\paper_reimpl\logs\02_sample_stage_a_ttf_v5_gpu0_%DT%.log
