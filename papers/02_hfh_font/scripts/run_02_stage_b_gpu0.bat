...
set TRAIN=%PAPER_DIR%\src\hfh_font\configs\train_stage_b_midtrain.yaml
set INIT=%PAPER_DIR%\outputs\stage_a_ttf_v5\hfh_font_last.pt
for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddHHmmss"') do set DT=%%I
set LOG=D:\Char\ayueh\paper_reimpl\logs\02_stage_b_gpu0_%DT%.log
