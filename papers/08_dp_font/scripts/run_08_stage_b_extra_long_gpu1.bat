...
chcp 65001 >nul
setlocal
REM 08 SB extra-long 150k continuation on cuda:1, warm-start from PINN=0 50k ckpt.
...
set TRAIN=%PAPER_DIR%\src\dp_font\configs\train_stage_b_extra_long.yaml
set INIT=%PAPER_DIR%\outputs\stage_b_long_pinn0\dp_font_last.pt
for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddHHmmss"') do set DT=%%I
set LOG=D:\Char\ayueh\paper_reimpl\logs\08_stage_b_extra_long_gpu1_%DT%.log
