...
chcp 65001 >nul
setlocal
REM 08 Stage A v3 — 200k step continuation from v2 ckpt at lr=1e-5 (cuda:0).
...
set TRAIN=%PAPER_DIR%\src\dp_font\configs\train_stage_a_ttf_v3.yaml
set INIT=%PAPER_DIR%\outputs\stage_a_ttf_v2\dp_font_last.pt

for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddHHmmss"') do set DT=%%I
set LOG=D:\Char\ayueh\paper_reimpl\logs\08_stage_a_ttf_v3_gpu0_%DT%.log
