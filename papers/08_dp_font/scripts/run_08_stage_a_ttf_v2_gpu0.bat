...
chcp 65001 >/dev/null
setlocal
REM 08_dp_font Stage A v2 — 200k step warm-start from v1 ckpt (cuda:0).
...
set TRAIN=%PAPER_DIR%\src\dp_font\configs\train_stage_a_ttf_v2.yaml
set INIT=%PAPER_DIR%\outputs\stage_a_ttf_v1\dp_font_last.pt

for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMddHHmmss"') do set DT=%%I
set LOG=D:\Char\ayueh\paper_reimpl\logs\08_stage_a_ttf_v2_%DT%.log
