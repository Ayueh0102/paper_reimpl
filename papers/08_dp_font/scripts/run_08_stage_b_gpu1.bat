...
chcp 65001 >nul
setlocal
REM 08 Stage B on cuda:1 (cuda:0 idle). Uses enriched manifest with absolute paths.
...
set TRAIN=%PAPER_DIR%\src\dp_font\configs\train_stage_b_midtrain.yaml
...
set LOG=D:\Char\ayueh\paper_reimpl\logs\08_stage_b_gpu1_%DT%.log
